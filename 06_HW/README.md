| Фамилия      | Class | 1   | 2   | Память | Tot  |
| ------------ | ----- | --- | --- | ------ | ---- |
| Криворученко | 1.0   | 1.0 | 1.0 |   1.0  | 100% |
...
public:
    ...
    // like St% and group <> 209 order by group, name;"
    // parsed to
...
* Применять команду к списку и выводить только найденные в `select` элементы в стандартный вывод (`stdout`), используя в тех запросах, где это возможно, структуры для быстрого поиска объектов, и линейный просмотр списка в тех запросах, где это невозможно.
...
* `insert (<name>, <phone>, <group>);` — добавить объект типа `record` с указанными полями в список, если такой элемент не существует.
* `select <условия на выводимые поля> [where <условие поиска>] [order by <условия сортировки>];` — вывести элементы списка, удовлетворяющие указанным условиям поиска и в указанном порядке.
* `delete [where <условие поиска>];` — удалить элементы списка, удовлетворяющие указанным условиям. При отсутствии `where` удаляется весь список.
...
* Если значения первого поля равны, сортировать по следующему полю, и так далее.
...
Если участвует более одного условия, они задают условия на разные поля записи `record`.
...
* `<поле>` — только символьное поле, то есть `name`
* `<образец>` — шаблон поиска
...
* `[n-m]` — один символ из диапазона `n...m`
* `\[`, `\]`, `\\` — литеральные символы `[`, `]`, `\`
* `[bn-m]` — любой символ, код которого не содержится в диапазоне `n...m`
...
Условие выполнено, если поле не соответствует образцу.
...
Добавить запись с указанными полями, если такой не существует.

```text
select group, name where phone = 1234567 and name = Student;
```

Вывести поля `group` и `name` для всех элементов, у которых `phone = 1234567` и `name = Student`.

```text
select * where phone >= 1234567 and name like St% order by group;
```

Вывести все поля для всех элементов, у которых `phone >= 1234567` и `name` соответствует образцу `St%`, отсортировав по `group`.
...
Программа должна получать параметры через аргументы командной строки и стандартный ввод.
...
* `commands.txt` подаётся на стандартный ввод
...
В указанном файле данные имеют формат:
...
* длина слова неизвестна заранее
* память под слово выделяется динамически
* все записи в файле различны
...
Формат должен соблюдаться точно, чтобы можно было автоматизировать обработку результатов.
...
9. Хеш-реализация по указанным `k` первым буквам имени на базе массива списков объектов
10. Хеш-реализация по указанным `k` первым цифрам номера телефона на базе массива списков объектов
11. Хеш-реализация по указанным `k` последним буквам имени на базе массива списков объектов
12. Хеш-реализация по указанным `k` последним цифрам номера телефона на базе массива списков объектов
13. Хеш-реализация по сумме букв имени по указанному модулю `k` на базе массива списков объектов
14. Хеш-реализация по сумме цифр номера телефона по указанному модулю `k` на базе массива списков объектов
15. Хеш-реализация по указанным `k` первым буквам имени на базе динамического вектора векторов длины `m`
16. Хеш-реализация по указанным `k` первым цифрам номера телефона на базе динамического вектора векторов длины `m`
17. Хеш-реализация по указанным `k` последним буквам имени на базе динамического вектора векторов длины `m`
18. Хеш-реализация по указанным `k` последним цифрам номера телефона на базе динамического вектора векторов длины `m`
19. Хеш-реализация по сумме букв имени по указанному модулю `k` на базе динамического вектора векторов длины `m`
20. Хеш-реализация по сумме цифр номера телефона по указанному модулю `k` на базе динамического вектора векторов длины `m`
21. Хеш-реализация по указанным `k` первым буквам имени на базе B-дерева по основанию `m`
22. Хеш-реализация по указанным `k` первым цифрам номера телефона на базе B-дерева по основанию `m`
23. Хеш-реализация по указанным `k` последним буквам имени на базе B-дерева по основанию `m`
24. Хеш-реализация по указанным `k` последним цифрам номера телефона на базе B-дерева по основанию `m`
25. Хеш-реализация по сумме букв имени по указанному модулю `k` на базе B-дерева по основанию `m`
26. Хеш-реализация по сумме цифр номера телефона по указанному модулю `k` на базе B-дерева по основанию `m`
27. Хеш-реализация по указанным `k` первым буквам имени на базе AVL дерева
28. Хеш-реализация по указанным `k` первым цифрам номера телефона на базе AVL дерева
29. Хеш-реализация по указанным `k` последним буквам имени на базе AVL дерева
30. Хеш-реализация по указанным `k` последним цифрам номера телефона на базе AVL дерева
31. Хеш-реализация по сумме букв имени по указанному модулю `k` на базе AVL дерева
32. Хеш-реализация по сумме цифр номера телефона по указанному модулю `k` на базе AVL дерева
33. Хеш-реализация по указанным `k` первым буквам имени на базе красно-чёрного дерева
34. Хеш-реализация по указанным `k` первым цифрам номера телефона на базе красно-чёрного дерева
35. Хеш-реализация по указанным `k` последним буквам имени на базе красно-чёрного дерева
36. Хеш-реализация по указанным `k` последним цифрам номера телефона на базе красно-чёрного дерева
37. Хеш-реализация по сумме букв имени по указанному модулю `k` на базе красно-чёрного дерева
38. Хеш-реализация по сумме цифр номера телефона по указанному модулю `k` на базе красно-чёрного дерева

---

## Файл `config.txt`

Дополнительные параметры (`k`, `m`) должны считываться из файла `config.txt`, который находится в том же каталоге, что и исполняемый файл.

### Формат файла

* строки, начинающиеся с `#`, игнорируются
* пустые строки игнорируются
* параметрами являются целые числа, разделённые пробельными символами или переводом строки
...
## Пример кода для получения полного пути к конфигурационному файлу
